...
import { AlertTriangle, LogOut, Trash2, CheckCircle } from 'lucide-react';
...
interface ConfirmationModalProps {
    ...
    showCancel?: boolean;
    type?: ConfirmType;
    icon?: 'trash' | 'logout' | 'alert' | 'success';
}
...
const ConfirmationModal: React.FC<ConfirmationModalProps> = ({
    ...
    const getColors = () => {
        switch (type) {
            ...
            case 'success': return 'bg-emerald-50 text-emerald-600 hover:bg-emerald-600 hover:text-white border-emerald-200';
    ...
    const renderIcon = () => {
        const className = `p-3 rounded-full ${type === 'danger' ? 'bg-red-100 text-red-600' : type === 'warning' ? 'bg-orange-100 text-orange-600' : type === 'success' ? 'bg-emerald-100 text-emerald-600' : 'bg-blue-100 text-blue-600'} mb-4`;
        ...
        switch (icon) {
            ...
            case 'success': return <div className={className}><CheckCircle size={24} /></div>;
