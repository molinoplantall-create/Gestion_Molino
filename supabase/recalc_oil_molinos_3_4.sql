-- ============================================================
-- Recálculo de horas de cambio de aceite (Molinos III y IV)
-- Fecha base: 02-07-2026
-- Límite establecido: 500 horas
-- ============================================================
...
-- 2. Actualizamos SOLO Molino III y Molino IV, restando de 500
UPDATE mills
SET 
  hours_to_oil_change = GREATEST(0, ROUND((500 - COALESCE(hsc.sum_hours, 0))::numeric, 2))
FROM mills m2
LEFT JOIN hours_since_change hsc ON hsc.mill_id = m2.id
WHERE mills.id = m2.id
  AND m2.name IN ('Molino III', 'Molino IV', 'MOLINO III', 'MOLINO IV');
...
SELECT 
  ...
  hours_to_oil_change 
FROM mills 
WHERE name IN ('Molino III', 'Molino IV', 'MOLINO III', 'MOLINO IV');
